...
// Date Oct Tue 08 18:34:00 CST 2024.

#ifndef VAR_BUILTIN_TABBED_H
#define VAR_BUILTIN_TABBED_H
...
struct TabInfo {
    ...
    std::string path;
    bool valid() const {
        return !tab_name.empty() && !path.empty();
    }
};
...
public:
    ...
    const TabInfo& operator[](size_t i) const {
        return _list[i];
    }
