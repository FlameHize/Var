...
TEST_F(StringSplitterTest, sanity) {
    ...
    var::StringSplitter ss2(str, ' ', var::ALLOW_EMPTY_FIELD);
...
TEST_F(StringSplitterTest, number_list) {
    ...
    var::StringMultiSplitter ss(str, ", ");
    ASSERT_TRUE(ss);
    ASSERT_EQ(3ul, ss.length());
    ASSERT_FALSE(strncmp(ss.field(), "123", ss.length()));
    ...
    var::StringMultiSplitter ss2(str, str + str_len, ", ");
    ASSERT_TRUE(ss2);
    ASSERT_EQ(3ul, ss2.length());
...
TEST_F(StringSplitterTest, split_limit_len) {
    ...
    var::StringSplitter ss(str, str + 5, '\t');
    ...
    var::StringSplitter ss2(str, str + 5, '\0');
    ...
    var::StringPiece sp(str, 5);
    // Allows using '\0' as separator
    var::StringSplitter ss3(sp, '\0');
...
TEST_F(StringSplitterTest, key_value_pairs_splitter_sanity) {
    ...
    for (int i = 0 ; i < 3; ++i) {
        ...
        var::KeyValuePairsSplitter* psplitter = NULL;
        if (i == 0) {
            psplitter = new var::KeyValuePairsSplitter(kvstr, '&', '=');
        } else if (i == 1) {
            psplitter = new var::KeyValuePairsSplitter(
                    kvstr.data(), kvstr.data() + kvstr.size(), '&', '=');
        } else if (i == 2) {
            psplitter = new var::KeyValuePairsSplitter(kvstr.c_str(), '&', '=');
        }
        var::KeyValuePairsSplitter& splitter = *psplitter;
