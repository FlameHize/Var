...
#ifndef VAR_BASE_STRINGSPLITTER_INL_H
#define VAR_BASE_STRINGSPLITTER_INL_H
...
StringSplitter::StringSplitter(const StringPiece& input, char sep,
                               EmptyFieldAction action)
    : StringSplitter(input.data(), input.data() + input.size(), sep, action) {}
...
void KeyValuePairsSplitter::UpdateDelimiterPosition() {
    ...
    if (_delim_pos == StringPiece::npos()) {
        _delim_pos = key_value_pair.size();
    }
}

}  // namespace var

#endif  // VAR_BASE_STRINGSPLITTER_INL_H
