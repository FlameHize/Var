...
TEST(HttpServerTest, resolved_incomplete_request)
{
    // // Test success.
    // InetAddress addr(2008);
    // EventLoop loop;
    ...
    //     TcpConnectionPtr conn = client.connection();
    //     if(count % 2 == 0) {
    //         conn->send(http_request_2, strlen(http_request_2));
    //     }
    //     else {
    //         conn->send(http_request_1, strlen(http_request_1));
    //     }
    //     ++count;
    // });
    // loop.loop();
...
TEST(HttpServerTest, resolved_one_more_request)
{
    // // It will loss one infomation.
    // InetAddress addr(2008);
    // EventLoop loop;
    // HttpServer server(&loop, addr, "httpserver");
    // server.Start();
...
TEST(HttpServerTest, resolved_url_path)
{
    std::string url("/vars/bthread_count");
    StringSplitter splitter(url, '/');
    std::string unresolved_path;
    HttpServer::FillUnresolvedPath(&unresolved_path, url, splitter);
    ASSERT_EQ(unresolved_path, "vars/bthread_count");
    
    ++splitter;
    HttpServer::FillUnresolvedPath(&unresolved_path, url, splitter);
    ASSERT_EQ(unresolved_path, "bthread_count");
}